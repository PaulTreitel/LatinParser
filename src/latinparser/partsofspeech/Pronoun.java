...
public class Pronoun implements Word {
	...
	public Pronoun(String origin, String c) {
		...
		meaning = "";
		part = "";
		addPossForm(LatinParser.PRONOUNS);
	}
	
	public void addPossForm(String entry) {
		try {
			Scanner sc = new Scanner(new File(entry));
			sc.useDelimiter("$^");
			String[] pronouns = sc.next().split("\r\n\r\n");
			sc.close();
			if (!getForm(pronouns, word)) {
				getForm(pronouns, word.substring(0, word.length()-3));
			}
		} catch (FileNotFoundException e) {
			System.out.println("Pronoun file "+ entry +" does not exist.");
			e.printStackTrace();
		}
	}
